...
import {getAllMoviesByGenreCurrentPagePopularityAscThunk} from "../../store/slices/genres.slice";
...
const GenreMoviesPopularityAsc: FC = () => {
    ...
    useEffect(() => {
        dispatch(getAllMoviesByGenreCurrentPagePopularityAscThunk({id: state, currentPage}))
    }, [currentPage]);
    ...
    return (
        <div>
            ...
            <div className={'movies-card-box'}>{results && results.map((movie: IMovieCurrentPage) => <MovieCurrentPage key={movie.id}
                                                                                       movie={movie}/>)}</div>
        </div>
    );
};

export default GenreMoviesPopularityAsc;